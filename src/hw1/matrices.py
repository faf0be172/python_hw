...
def get_transpose(matrix):
    if validate_matrix(matrix):
        ...
        assert len(transposed_matrix) == len(matrix[0]) and \
               len(transposed_matrix[0]) == len(matrix), "Result matrix dimension is invalid"
...
def get_matrices_sum(matrix_a, matrix_b):
    if validate_matrix(matrix_a) and validate_matrix(matrix_b):
        if len(matrix_a) == len(matrix_b) and len(matrix_a[0]) == len(matrix_b[0]):
            try:
                result = [tuple(x + y for (x, y) in zip(row_a, row_b))
            ...
            assert len(result) == len(matrix_a) and \
                   len(result[0]) == len(matrix_a[0]), "Result matrix dimension is invalid"
            return result
        else:
...
def get_matrices_product(matrix_a, matrix_b):
    if validate_matrix(matrix_a) and validate_matrix(matrix_b):
        ...
        try:
            result = [tuple(sum(x * y for (x, y) in zip(row_a, row_b))
        ...
        assert validate_matrix(result), "Result is not a matrix"
        assert len(result) == len(matrix_a) and \
               len(result[0]) == len(matrix_b[0]), "Result matrix dimension is invalid"
        return result
